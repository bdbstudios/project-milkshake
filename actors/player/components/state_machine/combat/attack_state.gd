...
func enter() -> void:
	...
	player_body.animation_tree.animation_finished.connect(_on_attack_animation_finished)
	player_body.animation_tree.set(animation_parameter_name, AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE)

	player_body.disable_motion()
	player_body.hitbox_collision.disabled = false
...
func physics_update(delta: float) -> void:
	player_body.face_camera_direction(delta)
...
func exit() -> void:
	player_body.enable_motion()
	player_body.hitbox_collision.disabled = true
	player_body.animation_tree.animation_finished.disconnect(_on_attack_animation_finished)
